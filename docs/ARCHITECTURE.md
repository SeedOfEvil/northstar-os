...
Planned services include launcher, notifications, settings, file associations, volumes, updates, and power. The notification service is an in-process, bounded history of user-visible launch events that never requires root access or supervises applications; its history is written to a user-private, owner-only file and restored at login, bounded by an entry cap and a retention window, and a malformed record is skipped rather than allowed to stop the shell. The settings surface is a catalog every control is declared into, naming the controller accessor that reads, writes, or performs it, so a control whose accessors cannot back it is refused registration rather than rendered dead. File associations are likewise stored as user-scoped extension preferences and are validated against the current launcher catalog before use. Reboot, shutdown, and radio switching are controlled authorization operations, not arbitrary shell commands. Each goes through a fixed-argument boundary that enumerates the exact actions it permits and resolves its own targets, so no caller-supplied string reaches a privileged command line; a surface offers the control only where that boundary is installed and the hardware backing it is present. The shell additionally listens on a user-private control socket so the compositor can deliver a global shortcut regardless of which client holds keyboard focus; it acts only on a fixed set of command names and never executes text it is given. Project `.app` bundles carry a source/package/revision provenance record that the launcher validates before discovery. The Software surface exposes an attributed `pkg` inventory, the pinned `FreeBSD-ports` catalogue, and a separate provenance-aware Northstar publication. Install/remove authorization carries only a short-lived opaque plan identifier; the root-owned runner refreshes the fingerprint-authenticated catalogue, independently recovers the operation and target, reproduces the exact preview, and creates a boot environment before mutation. Dependencies, locked packages, `pkg` itself, FreeBSD base/kernel packages, and packages from unknown repositories cannot be directly removed. Northstar updates retain their separate verified request, broker, transaction, and rollback state.
...
FreeBSD base and kernel updates use the official FreeBSD mechanism. Third-party desktop dependencies and user-selected applications come from the release-pinned, fingerprint-authenticated `FreeBSD-ports` source. Northstar components are built in clean Poudriere jails and published through a separate signed `pkg` repository. Each third-party plan is bound to the refreshed catalogue digest, exact source/origin/version record, and complete dry-run preview; mutation creates a ZFS boot environment first and records rollback state without touching `/home`. The Northstar publication sidecar independently records repository revision, target ABI, package origins, source inputs, and project revisions; its signature envelope does not replace `pkg` catalogue files or either repository's native signature path.
