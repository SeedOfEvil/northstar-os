...
Version 0.1, accepted for design and implementation planning. This is a Northstar presentation and portability format. It is not a promise of macOS compatibility and it does not replace FreeBSD packages.
...
The launcher prefers a valid `.desktop` entry when one is available, then discovers project bundles from declared application roots. A bundle can expose a `.desktop` entry for compatibility with existing FreeBSD tools.
