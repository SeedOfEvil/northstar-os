...
Before starting a milestone RC assembly, run the real FreeBSD storage reset
gate on the disposable builder:

```sh
sudo make installer-zfs-reset-smoke
```

The file-backed gate is necessary but not sufficient. Before another installer
image is assembled, attach one explicitly disposable VirtIO-SCSI disk to the
development VM and run the hardware-shaped retry regression with its exact
provider name and byte size:

```sh
sudo make installer-virtio-retry-smoke \
  DEVICE=<daN> EXPECTED_BYTES=<exact-byte-size>
```

The command refuses disks that do not match the typed provider, exact size,
QEMU identity, unused GEOM mode, and empty mount/pool/swap/GPT state. It must
prove both that an EFI filesystem mounted through a GEOM label alias is treated
as target usage and that a retry after an already-cleared ZFS label can replace
the GPT. The fixture removes its GPT and restores GEOM protection on exit.
...
The QCOW2 assembler treats the resolved primary Northstar package as
authoritative. A Northstar package captured in an older runtime closure is
dependency evidence only and must be replaced during assembly. Assembly must
fail unless the exact locked version is installed and the installed installer
executor digest matches that package. Image provenance records the effective
package set after this replacement, not the stale input closure.
...
The rejected r83 package, signed repository, assembly, public-export,
snapshot-only boot evidence, and stale-runtime root cause are recorded in
