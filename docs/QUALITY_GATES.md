...
| M7 Files v3 acceptance | At 1280x800, Files tabs independently restore Home, Trash, nested Home, and mounted-volume locations; Ctrl+L navigation stays inside validated boundaries; copy/move work asynchronously without freezing input; conflicts never overwrite and require Keep Both; copy from mounted volumes is allowed but Cut/mutation is rejected; one-step undo restores a move or sends a copied result through recoverable Trash; controls and feedback remain unclipped |
...
| M6 evidence bundle | Atomic new output directory; exactly three bounded source records plus schema-1 summary and SHA-256 manifest; cross-record lane/status/digest agreement; tamper, unknown file, missing file, duplicate field, and VM physical-pass rejection; mode-0700 directory and mode-0600 files; verification performs no mutation |
