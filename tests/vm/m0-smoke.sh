...
# M0 native smoke test. Run this as the unprivileged development user from
...
Check the M0 package/session prerequisites. With --launch, briefly start
...
for command_name in pkg qterminal firefox xterm wayfire dbus-run-session Xwayland; do
...
for package_name in wayfire xwayland qterminal firefox xterm; do
...
printf '%s\n' 'PASS: M0 packages and Wayland/Xwayland session variables are present'
