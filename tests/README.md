...
`vm/package-mutation-smoke.sh` exercises the third-party install/remove runner
with isolated fake `pkg`, `bectl`, and `zfs` tools. It binds a short-lived opaque
identifier to the exact authenticated-catalogue record and dry-run preview,
rejects replay/catalogue/lock failures, proves create-before-mutation ordering,
and schedules boot-environment rollback after injected failure.
