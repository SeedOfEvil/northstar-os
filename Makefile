...
NORTHSTAR_VERSION := $(shell sed -n '1p' VERSION)
...
NORTHSTAR_IMAGE_PACKAGE ?= .artifacts/m5-inputs/northstar-$(NORTHSTAR_VERSION)-amd64.pkg
...
help:
	...
	@printf '%s\n' '  make radio-helper-test  Test fixed radio authorization and root mutation boundary'
	...
	@printf '%s\n' '  make package-mutation-smoke  Prove opaque install/remove plans with isolated tools (root)'
...
package-mutation-smoke:
	@sh tests/vm/package-mutation-smoke.sh
